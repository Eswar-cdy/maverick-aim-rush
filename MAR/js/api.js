...
async function apiFetch(url, options = {}) {
    ...
    if (hadAuthHeader && isTokenExpired()) {
        ...
        if (refreshed) {
            ...
            console.log('❌ Proactive refresh failed, clearing tokens...');
            clearTokens();
        }
...
async function calculateFitnessMetrics(goalType = 'maintenance') {
    try {
        const response = await apiFetch('/calculator/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function fetchMacroTarget(date = null) {
    try {
        const url = date ? `/macro-targets/?date=${date}` : '/macro-targets/';
        const response = await apiFetch(url);
...
async function createMacroTarget(goalType = 'maintenance') {
    try {
        ...
        const response = await apiFetch('/macro-targets/', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
async function fetchCalculationHistory() {
    try {
        const response = await apiFetch('/calculator-results/');
